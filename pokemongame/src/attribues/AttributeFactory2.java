...
public class AttributeFactory2 {
		...
		for (final String[] values : output) {
			...
			switch (values[TYPE_LOC].trim().toLowerCase()) {
				case "integer": {
					...
					addAttributeDetails(values, integerAttribute, AttributeFactory.INTEGER);
					AttributeFactory.INTEGER.addToAttributesManager(name, integerAttribute);
				...
				case "double": {
					...
					addAttributeDetails(values, doubleAttribute, AttributeFactory.DOUBLE);
				...
				case "string": {
					...
					addAttributeDetails(values, stringAttribute, AttributeFactory.STRING);
	...
	private static <T> void addAttributeDetails(final String[] values, final Attribute<T> attribute, final AttributeFactory<?> parseType) {
	...
	public static AttributeFactory2 getInstance() {
		return INSTANCE;
	}
	/*
	...
	public <T> Attribute<T> generateAttribute(final String name, final T value, final AttributeFactory<T> parseType) {
